...
def model(name="Model", actions=2, beta=0, size=4):
    with tf.name_scope(name):
        obs = tf.placeholder(tf.float32, shape=[None, size * size], name="Observation_Input")

        with tf.name_scope("value"):
            v_net = tflearn.fully_connected(obs, 32, activation="relu")
            v_net = tflearn.fully_connected(v_net, 32, activation="relu")
            value = tflearn.fully_connected(v_net, 1, activation="linear", name="Value")
        with tf.name_scope("policy"):
            p_net = tflearn.fully_connected(obs, 32, activation="relu")
            p_net = tflearn.fully_connected(p_net, 32, activation="relu")
