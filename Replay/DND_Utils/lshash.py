...
class LSHash(object):
    ...
    def index(self, input_point, extra_data=None):
        ...
        # if type(input_point) is np.ndarray:
            # print("numpy array")
    ...
    def query_no_data(self, query_point, num_results=None, distance_func=None):
        """ Takes `query_point` which is either a tuple or a list of numbers,
        returns `num_results` of results as a list of tuples that are ranked
        based on the supplied metric function `distance_func`.
        ...
        d_func = self.euclidean_dist_square_numpy

        for i, table in enumerate(self.hash_tables):
            binary_hash = self._hash(self.uniform_planes[i], query_point)
            candidates_set.update(table.get_list(binary_hash))

        # rank candidates by distance function
        candidates = [(ix, d_func(query_point, ix))
                      for ix in candidates_set]
        # if len(candidates_set) > 0:
        #     ix_array = [ix for ix in candidates_set]
        #     candidates_array = np.stack(ix_array)
        #     all_dot = np.dot(candidates_array, query_point)
        #     candidates = list(zip(ix_array, all_dot))
        # else:
        #     candidates = []
    ...
    @staticmethod
    def euclidean_dist_square_numpy(x, y):
        diff = x - y
