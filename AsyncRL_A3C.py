...
with tf.Graph().as_default():
    with tf.Session(config=config) as sess:
        ...
        global_policy_loss = global_model["Policy_Loss"]
        global_value_loss = global_model["Value_Loss"]
        ...
        threads = [threading.Thread(target=actor, args=(envs[i], global_model, i + 1, EPISODE_T_MAX, sess, update_global_model_op, None)) for i in range(THREADS)]
