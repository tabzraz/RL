...
class ExperienceReplay_Sarsa:
    ...
    Experience = collections.namedtuple("Experience", "state action reward state_next action_next steps terminal pseudo_reward density pseudo_reward_t trajectory_end")
    ...
    def Add_Exp(self, state_now, action, reward, state_after, action_after, steps, terminal, pseudo_reward=0, density=1):
        ...
        new_exp = self.Experience(state=np.copy(state_now), action=action, reward=reward, state_next=np.copy(state_after), action_next=action_after ,steps=steps, terminal=terminal, pseudo_reward=pseudo_reward, density=density, pseudo_reward_t=self.T, trajectory_end=terminal)
