...
class DeadlyGridWorld(gym.Env):
    ...
    def __init__(self):
        ...
        # self.negative_reward = -0.005#-10.0 * (self.goals) / self.limit
        self.negative_reward = 0 # Can't have a negative reward with spikes
        self.spike_reward = -1
    ...
    def _step(self, a):
        ...
        if self.grid[new_player_pos] == 1:
            new_player_pos = self.player_pos
        # Into a goal
        elif self.grid[new_player_pos] == 4:
            r += self.spike_reward
            finished = True
        ...
        return self.grid[:, :, np.newaxis] / 4, r, finished, info_dict
        ...
        if mode == "rgb_array":
            ...
            for x in range(grid.shape[0]):
                for y in range(grid.shape[1]):
                    if grid[x, y] != 0:
                        image[x, y] = (255 * grid[x, y] / 4, 255 * grid[x, y] / 4, 255 * grid[x, y] / 4)
            return image
        else:
            pass
    ...
    def state_to_player_pos(self, state):
        ...
        goals_list = [internal_state[g[0], g[1]] > 0.45 and internal_state[g[0], g[1]] < 0.55 for g in self.goals_order]
        # print(goals_list)
        # print(internal_state)
        player_pos = list(np.argwhere(internal_state == 0.75)[0])
        joint = player_pos + goals_list
        # print(tuple(joint))
        return tuple(joint)
    ...
    def trained_on_states(self, player_visits, args):
        ...
        for goal in self.goals_order:
            canvas[goal[0], goal[1], :] = 2 / 4
        if self.num_goals >= 2:
            for g in range(self.num_goals):
                for go_i, goal in enumerate(self.goals_order):
                    if go_i != g:
                        canvas[goal[0] + grid_x, goal[1] + g * grid_y, :] = 2 / 4
        if self.num_goals >= 3:
            for g in range(self.num_goals):
                for go_i, goal in enumerate(self.goals_order):
                    if go_i == g:
                        canvas[goal[0] + 2 * grid_x, goal[1] + g * grid_y, :] = 2 / 4
                ...
                if self.grid[x, y] == 1:
                    ...
                    for zx in range(1, self.num_goals):
                        for zy in range(self.num_goals):
                            canvas[zx * grid_x + x, zy * grid_y + y, :] = 1 / 4
    ...
    def frontier(self, exp_model, args, max_bonus=None):
        ...
        for a in range(actions):
            for x in range(grid_x):
                for y in range(grid_y):
                    ...
                    state_copy[x, y] = 3
                    # print(state_copy)
                    state_copy = state_copy[:, :, np.newaxis] / 4
        ...
        for a in range(actions):
            for x in range(grid_x):
                for y in range(grid_y):
                    if grid[x, y] == 1 or grid[x, y] == 2:
                        canvas[x, y + (grid_y * a), :] = grid[x, y] / 4
